...
class TestPollsManagement(TestCase):
    ...
    def test_poll_can_be_created(self):
        ...
        self.assertEqual(created_poll.title, 'The Title')
        self.assertEqual(created_poll.description, 'The Description')
        self.assertEqual(created_poll.is_published, False)
        self.assertEqual(created_poll.user.id, self.user.id)
        self.assertEqual(created_poll.choice_set.count(), 2)
        self.assertEqual(created_poll.choice_set.all()[0].title, 'first')
        self.assertEqual(created_poll.choice_set.all()[1].title, 'last')
        ...
        self.assertEqual(created_poll.title, 'The Title')
        self.assertEqual(created_poll.description, None)
        self.assertEqual(created_poll.is_published, False)
        self.assertEqual(created_poll.user.id, self.user.id)
        self.assertEqual(created_poll.choice_set.count(), 2)
        self.assertEqual(created_poll.choice_set.all()[0].title, 'a')
        self.assertEqual(created_poll.choice_set.all()[1].title, 'b')

    def test_poll_can_be_deleted(self):
        poll = Poll(title='hello', user=self.user2)
        poll.save()

        self.assertEqual(self.client.post('/polls/delete/', {}).status_code, 401)
        self.assertEqual(self.client.post('/polls/delete/', {}, HTTP_TOKEN='test').status_code, 404)
        self.assertEqual(self.client.post('/polls/delete/', {'poll_id': 12356}, HTTP_TOKEN='test').status_code, 404)
        self.assertEqual(self.client.post('/polls/delete/', {'poll_id': poll.id}, HTTP_TOKEN='test').status_code, 403)

        self.assertEqual(self.client.post('/polls/delete/', {'poll_id': poll.id}, HTTP_TOKEN='test2').status_code, 200)
